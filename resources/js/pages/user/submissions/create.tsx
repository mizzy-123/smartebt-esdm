...
export default function SubmissionsCreate({ entryType, entryTypeLabel, categories }: CreateProps) {
    ...
    setLayoutProps({
        breadcrumbs: [
            ...
            { title: entryTypeLabel, href: `/submissions/create?type=${entryType}` },
        ],
    });

    const { data, setData, post, processing, errors } = useForm({
        entry_type: entryType,
        category: isTerbangun ? (categories[0]?.value ?? 'plts') : '',
        lokasi: '',
        desa: '',
        kecamatan: '',
        kabupaten: '',
        ...
        <div className="mx-auto max-w-3xl p-6 lg:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-border bg-white p-6 shadow-sm">
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    <div className="space-y-1.5">
                        <Label>Desa</Label>
                        <Input value={data.desa} onChange={(e) => setData('desa', e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex h-9 items-center rounded-md border border-border bg-muted/40 px-3 text-sm font-medium">
                                    {bauranPreview !== null ? bauranPreview.toLocaleString('id-ID', { maximumFractionDigits: 6 }) : '—'}
                                </div>
                            </div>
                        </div>
                ...
                <div className="space-y-1.5">
                    ...
                    {errors.foto_kondisi && <p className="text-xs text-destructive">{errors.foto_kondisi}</p>}
                </div>
                ...
                <div className="space-y-1.5">
                    ...
                    {(errors.latitude || errors.longitude) && (
                        <p className="text-xs text-destructive">{errors.latitude || errors.longitude}</p>
                    )}
                </div>
