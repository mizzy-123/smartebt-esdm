...
const WILAYAH_KEYS = ['kabupaten', 'kecamatan', 'desa'] as const;
    ...
    return (
        <div className="mx-auto max-w-3xl p-6 lg:p-8">
            <div className="mb-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Perbaiki field yang ditolak admin, lalu kirim ulang.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl border border-border bg-white p-6 shadow-sm">
                {WILAYAH_KEYS.some((key) => isRejected(key)) && (
                    <WilayahSelectFields
                ...
                {Object.entries(fields)
                    ...
                    .filter(([key]) => !WILAYAH_KEYS.includes(key as (typeof WILAYAH_KEYS)[number]))
                    .map(([key, label]) => {
                        if (key === 'bauran_energi') {
                            return (
                                <div key={key} className="space-y-1.5">
                                    <Label>{label} (otomatis)</Label>
                                    <div className="rounded-md border bg-muted/40 px-3 py-2 text-sm">{bauranPreview ?? '—'}</div>
                                </div>
                            );
                        }
                        ...
                        if (key === 'deskripsi_titik') {
                            return (
                                <div key={key} className="space-y-1.5">
                                    <Label>{label}</Label>
                                    <Textarea value={data.deskripsi_titik} onChange={(e) => setData('deskripsi_titik', e.target.value)} />
                                </div>
                            );
                        }
