...
class SubmissionPresenter
{
    ...
    public static function toArray(Submission $submission, bool $includeUser = false): array
    {
        ...
        $data = [
            'id' => $submission->id,
            'entry_type' => $submission->entry_type?->value ?? EntryType::Potensi->value,
            'entryTypeLabel' => $submission->entry_type?->label() ?? EntryType::Potensi->label(),
            'category' => $submission->category?->value,
            'categoryLabel' => $submission->category?->label()
                ?? ($submission->entry_type === EntryType::Potensi ? 'Potensi Lokal EBT' : '-'),
